 ...
 * - differs, no force -> write "<target>.astrostack-new" and report "conflict"
 */
export async function writeComponentFile(
  ...
  if (existsSync(abs)) {
    ...
    if (!force) {
      const altAbs = `${abs}.astrostack-new`;
      await mkdir(dirname(altAbs), { recursive: true });
      await writeFile(altAbs, content, "utf8");
      return { target, outcome: "conflict", altPath: `${target}.astrostack-new` };
    }
